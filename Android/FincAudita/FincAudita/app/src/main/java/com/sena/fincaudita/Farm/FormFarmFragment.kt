...
class FormFarmFragment : Fragment() {
    ...
    private fun saveFarm(farm: Farm, crops: MutableList<Int>) {
        ...
        try {
            ...
            val builder = AlertDialog.Builder(requireContext())
            builder.setTitle(errorTitle)
                .setMessage("Error al guardar la granja: ${error.message}")
                .setPositiveButton("OK") { dialog, _ -> dialog.dismiss() }
                .create()
                .show()
        }
    ...
    private fun updateFarm(farm: Farm, crops: MutableList<Int>) {
        ...
        try {
            ...
            val request = JsonObjectRequest(
                ...
                { error ->
                    ...
                    if (error.networkResponse != null && error.networkResponse.statusCode in 400..599) {
                        ...
                        builder.setTitle(errorTitle)
                            .setMessage("Error al actualizar la finca: ${error.message}")
                            .setPositiveButton("OK") { dialog, _ -> dialog.dismiss() }
                            .create()
                            .show()
                    } else {
            ...
            val builder = AlertDialog.Builder(requireContext())
            builder.setTitle(errorTitle)
                .setMessage("Error al actualizar la granja: ${error.message}")
                .setPositiveButton("OK") { dialog, _ -> dialog.dismiss() }
                .create()
                .show()
        }
            ...
            val request = JsonObjectRequest(
                ...
                { error ->
                    ...
                    if (error.networkResponse != null && error.networkResponse.statusCode in 400..599) {
                        ...
                        builder.setTitle(errorTitle)
                            .setMessage("Error al eliminar la finca: ${error.message}")
                            .setPositiveButton("OK") { dialog, _ -> dialog.dismiss() }
                            .create()
                            .show()
                    } else {
